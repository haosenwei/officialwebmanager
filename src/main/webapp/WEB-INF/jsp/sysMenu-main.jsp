...
<script type="text/javascript">
	...
	function aceEditf(id) {
		aceEdit(id,"修改菜单表","${server}toEdit?${_csrf.parameterName}=${_csrf.token}","${server}doEdit?${_csrf.parameterName}=${_csrf.token}","sysMenu_edit_form","sysMenuMain");
	}
	//查看
	...
	//删除
	function aceDelF(id) {
		aceDel(id , 'sysMenuMain',"${server}del?${_csrf.parameterName}=${_csrf.token}");
	}
	...
	$('.page-content-area').ace_ajax('loadScripts',[],function() {
		...
		initMainTable("sysMenuMain","${server}search?${_csrf.parameterName}=${_csrf.token}",columns);
	...
	$('#sysMenuMain_aceAdd').on('click',function() {
		aceAdd("添加菜单表","${server}toAdd?${_csrf.parameterName}=${_csrf.token}","${server}doAdd?${_csrf.parameterName}=${_csrf.token}","sysMenu_add_form","sysMenuMain");
	});
	//批量删除
	$('#sysMenuMain_aceDels').on('click',function() {
		aceDels("确定删除？","${server}dels?${_csrf.parameterName}=${_csrf.token}","sysMenuMain");
	});
</script>