...
<script type="text/javascript">
	...
	$('#sysMenuDialog_aceAdd').on('click',function() {
		aceAdd("添加菜单表","${server}sys/menu/toAdd?${_csrf.parameterName}=${_csrf.token}","${server}sys/menu/doAdd?${_csrf.parameterName}=${_csrf.token}","sysMenu_add_form","sysMenuDialog");
	});
	//修改
	function aceEditf(id) {
		aceEdit(id,"修改菜单表","${server}sys/menu/toEdit?${_csrf.parameterName}=${_csrf.token}","${server}sys/menu/doEdit?${_csrf.parameterName}=${_csrf.token}","sysMenu_edit_form","sysMenuDialog");
	}
	//查看
