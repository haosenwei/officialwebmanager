...
<script type="text/javascript">
	...
	function aceEditf(id) {
		aceEdit(id,"修改菜单表","${server}sys/sysmenutoEdit?${_csrf.parameterName}=${_csrf.token}","${server}sys/sysmenudoEdit?${_csrf.parameterName}=${_csrf.token}","sysMenu_edit_form","sysMenuMain");
	}
	//查看
	function aceViewf(id) {
		aceView(id,"查看菜单表","${server}sys/sysmenutoView?${_csrf.parameterName}=${_csrf.token}");
	}
	
	//删除
	function aceDelF(id) {
		aceDel(id , 'sysMenuMain',"${server}sys/sysmenudel?${_csrf.parameterName}=${_csrf.token}");
	}
	...
	$('.page-content-area').ace_ajax('loadScripts',[],function() {
		...
		initMainTable("sysMenuMain","${server}sys/sysmenusearch?${_csrf.parameterName}=${_csrf.token}",columns);
	...
	$('#sysMenuMain_aceAdd').on('click',function() {
		aceAdd("添加菜单表","${server}sys/sysmenutoAdd?${_csrf.parameterName}=${_csrf.token}","${server}sys/sysmenudoAdd?${_csrf.parameterName}=${_csrf.token}","sysMenu_add_form","sysMenuMain");
	});
	//批量删除
	$('#sysMenuMain_aceDels').on('click',function() {
		aceDels("确定删除？","${server}sys/sysmenudels?${_csrf.parameterName}=${_csrf.token}","sysMenuMain");
	});
</script>