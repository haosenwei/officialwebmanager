...
<script type="text/javascript">
	...
	function aceEditf(id) {
		aceEdit(id,"修改表字段","${server}toEdit?${_csrf.parameterName}=${_csrf.token}","${server}doEdit?${_csrf.parameterName}=${_csrf.token}","sysTableField_edit_form","sysTableFieldMain");
	}
	//查看
	function aceViewf(id) {
		aceView(id,"查看表字段","${server}toView?${_csrf.parameterName}=${_csrf.token}");
	}
	
	//删除
	function aceDelF(id) {
		aceDel(id , 'sysTableFieldMain',"${server}del?${_csrf.parameterName}=${_csrf.token}");
	}
	...
	$('.page-content-area').ace_ajax('loadScripts',[],function() {
		...
		initMainTable("sysTableFieldMain","${server}search?${_csrf.parameterName}=${_csrf.token}",columns);
	...
	$('#sysTableFieldMain_aceAdd').on('click',function() {
		aceAdd("添加表字段","${server}toAdd?${_csrf.parameterName}=${_csrf.token}","${server}doAdd?${_csrf.parameterName}=${_csrf.token}","sysTableField_add_form","sysTableFieldMain");
	});
	//批量删除
	$('#sysTableFieldMain_aceDels').on('click',function() {
		aceDels("确定删除？","${server}dels?${_csrf.parameterName}=${_csrf.token}","sysTableFieldMain");
	});
</script>